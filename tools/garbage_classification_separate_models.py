...
class LargeFeatureResNet(nn.Module):
    def __init__(self):
        """
        ...
        """
        super(LargeFeatureResNet, self).__init__()
    ...
    def forward(self, x):
        """
        Forward pass through the modified WideResNet.
...
class SmallFeatureResNet(nn.Module):
    def __init__(self):
        """
...
class GarbageClassifier(nn.Module):
    def __init__(self, num_classes=4):
        """
        Initialize the GarbageClassifier with two image models and one text model.

        Args:
            num_classes (int): Number of output classes for classification.
        """
        super(GarbageClassifier, self).__init__()
        # Image feature extraction models (one for large features, one for small features)
        self.large_feature_resnet = LargeFeatureResNet()
        self.small_feature_resnet = SmallFeatureResNet()

        # Text feature extraction with BERT
        self.bert = BertModel.from_pretrained('bert-base-uncased')

        # Separate classification layers for each image model
        self.fc_large_image = nn.Linear(512, num_classes)
        self.fc_small_image = nn.Linear(512, num_classes)

        # Classification layer for text model
    ...
    def forward(self, image, input_ids, attention_mask):
        ...
        Returns:
            tuple: Separate outputs for large image, small image, and text classifications.
        """
        # Large feature image model output
        large_image_features = self.large_feature_resnet(image)
        large_image_output = self.fc_large_image(large_image_features)

        # Small feature image model output
        small_image_features = self.small_feature_resnet(image)
        small_image_output = self.fc_small_image(small_image_features)
        ...
        return large_image_output, small_image_output, text_output
...
if __name__ == '__main__':
    ...
    # freeze all layers and unfreeze the last block
    for name, param in model.large_feature_resnet.named_parameters():
        if 'layer4' in name:
            param.requires_grad = True  # Unfreeze last ResNet block
        else:
            param.requires_grad = False  # Freeze other layers

    for name, param in model.small_feature_resnet.named_parameters():
        if 'layer4' in name:
            param.requires_grad = True  # Unfreeze last ResNet block
        else:
            param.requires_grad = False  # Freeze other layers
