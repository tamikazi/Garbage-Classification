...
def train():
    ...
    with wandb.init() as run:
        ...
        # Image transformations with potential hyperparameter tuning
        transform_train = transforms.Compose([
            transforms.Resize((224, 224)),
            transforms.RandomHorizontalFlip(p=config.get('horizontal_flip_prob', 0.5)),
            transforms.RandomRotation(degrees=config.get('rotation_degree', 10)),
            transforms.ToTensor(),
        ...
        # Initialize the model with dropout_rate from config
        ...
        for param in model.text_model.bert.parameters():
            param.requires_grad = False
        # Unfreeze the last encoder layer of BERT
        ...
        trainable_params = [
            # Image model layers with reduced learning rate
            ...
            # Text model: Last encoder layer with reduced learning rate
            {'params': model.text_model.bert.encoder.layer[-1:].parameters(), 'lr': config.learning_rate * 0.1},
...
def main():
    ...
    sweep_config = {
        ...
        'parameters': {
            ...
            'scheduler': {
                'values': [None, 'ReduceLROnPlateau', 'StepLR']
            },
            'seed': {
